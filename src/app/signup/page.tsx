...
import { Blocks, CheckCircle2, KeyRound, ShieldCheck, UserRound } from "lucide-react";
import { SignupForm } from "@/components/signup-form";

const steps = [
  { icon: UserRound, label: "Create the tenant owner account" },
  { icon: KeyRound, label: "Verify the owner email address" },
  { icon: ShieldCheck, label: "Invite your team with scoped roles" },
];

export default function SignupPage() {
  return (
    <main className="min-h-dvh bg-[#fffdf3] text-[#111111]">
      <div className="grid min-h-dvh lg:grid-cols-[minmax(420px,0.86fr)_minmax(560px,1.14fr)]">
          ...
          <div className="relative flex min-h-full w-full flex-col justify-between p-10 xl:p-14">
            ...
            <div className="max-w-xl">
              ...
              <h1 className="mt-6 text-5xl font-black leading-[0.98] tracking-tight xl:text-6xl">
                Start with a clean, governed workspace.
              </h1>
              <p className="mt-6 max-w-md text-base font-semibold leading-7 text-white/52">
                Create the owner account, verify the email, then bring the rest of your organization in with roles and permissions.
              </p>
            </div>
            ...
            <div className="grid gap-3">
              {steps.map(({ icon: Icon, label }, index) => (
                <div key={label} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.045] px-4 py-3">
                  <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-[#ffd400] text-sm font-black text-[#111111]">
                    {index + 1}
                  </span>
                  <Icon className="size-4 text-[#ffd400]" aria-hidden="true" />
          ...
          <div className="flex flex-1 items-center justify-center py-10">
            <div className="w-full max-w-[520px]">
              <div className="mb-7">
                ...
                <h2 className="mt-5 text-4xl font-black tracking-tight text-[#111111]">Create workspace</h2>
                <p className="mt-2 text-sm font-semibold leading-6 text-[#68645b]">
                  Keep this short: organization, owner details, password.
                </p>
              </div>
